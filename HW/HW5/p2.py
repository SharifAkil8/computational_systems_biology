...
# Isolate numerical data, first two columns which are strings
numerical_data = df.iloc[:, 2:]

# Split the data into ALL and AML
all_samples = numerical_data.iloc[:, :10]  # Assuming the first 10 are ALL samples
aml_samples = numerical_data.iloc[:, -10:] # Assuming the last 10 are AML samples
...
for index, row in numerical_data.iterrows():
    stat, p_val = stats.mannwhitneyu(all_samples.loc[index], aml_samples.loc[index], alternative='two-sided', use_continuity=True, method='exact')
    p_values.append(p_val)
    mean_diffs.append(np.median(aml_samples.loc[index]) - np.median(all_samples.loc[index]))

# Add p-values and median differences to the DataFrame
df['p_value'] = p_values
df['median_diff'] = mean_diffs
...
num_upregulated_in_aml = significant_genes[significant_genes['median_diff'] > 0].shape[0]
...
# significant_genes.to_excel('/Users/sharif/Desktop/UNDERGRAD_COLLEGE/Spring_2024/ECEN453/HW/HW5/significant_genes_p2.xlsx', index=False)
...
# p_values_df.to_excel('/Users/sharif/Desktop/UNDERGRAD_COLLEGE/Spring_2024/ECEN453/HW/HW5/p_values_p2.xlsx', index=False)
